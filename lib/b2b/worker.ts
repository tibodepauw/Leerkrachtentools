...
const ENV_KEYS = ["DATABASE_PATH", "SERVER_AI_DAILY_CALL_LIMIT", "DISCOVERY_DAILY_CALL_LIMIT", "ORG_AI_DAILY_LIMIT", "ORG_AI_GLOBAL_DAILY_LIMIT", "GROQ_API_KEY", "GROQ_MODEL", "CEREBRAS_API_KEY", "CEREBRAS_MODEL", "SAMBANOVA_API_KEY", "SAMBANOVA_BASE_URL", "SAMBANOVA_MODEL", "GOOGLE_GENERATIVE_AI_API_KEY", "GOOGLE_MODEL", "CLOUDFLARE_ACCOUNT_ID", "CLOUDFLARE_API_TOKEN", "CLOUDFLARE_MODEL"];
export async function runB2bJob(task: B2bTask, body: unknown, orgId: string, signal: AbortSignal) {
  ...
  try {
    const result = await runProcessJob<{ payload?: unknown; error?: boolean }>(workerEntry("b2b"), { task, body, orgId }, {
      ...
      env: Object.fromEntries(ENV_KEYS.flatMap((name) => process.env[name] !== undefined ? [[name, process.env[name]!]] : [])),
    });
