...
export async function extractDocumentText(buffer: Buffer, fileName: string, signal?: AbortSignal) {
  ...
  switch (extension) {
    ...
    case "docx":
      ...
      break;
    case "doc": {
      extracted = normalizeText((await new (createRequire(path.join(process.cwd(), "package.json"))("word-extractor"))().extract(source)).getBody());
      break;
    }
    case "pdf": {
      const { PDFParse } = createRequire(path.join(process.cwd(), "package.json"))("pdf-parse");
      const parser = new PDFParse({ data: source });
      try { extracted = normalizeText((await parser.getText()).text); }
